...
def mergedefined(*args: dict[str, Any]) -> dict[str, Any]:
    """
...
def dict2kwargs(
    ...
    """
    Create a string from a dict object using keyword argument syntax
        ...
        if isinstance(value, dict):
            ...
            for item in value:
                if isinstance(item, dict):
                    ...
                    sublist.append(dict2kwargs(dict(item)))
                else:
...
def getsoptions(**kwargs: Any) -> str:
    """
...
def parsesoptions(soptions: Optional[str]) -> dict[str, Any]:
    """
    ...
    while re.match(r'^[\w+-]+\s*=', soptions):
        ...
        if soptions.startswith('{'):
            ...
            if not match:
                raise ValueError('Unable to parse soptions: {}'.format(soptions))
            out[name] = re.split(r'\s+', match.group(1))
            soptions = match.group(2) or ''
        elif ' ' in soptions:
            out[name], soptions = re.split(r'\s+', soptions, 1)
        else:
            out[name] = soptions
            soptions = ''
    return out
...
def getlocale(locale: Optional[str] = None) -> str:
    """
    ...
    if locale:
        return locale
    return loc.getdefaultlocale()[0] or '<unknown>'
