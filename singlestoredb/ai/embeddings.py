...
def SingleStoreEmbeddingsFactory(
    ...
    **kwargs: Any,
    ...
    # out. A cached OpenAIEmbeddings instance reused across UDF calls can
    # otherwise retain an httpx connection whose underlying asyncio transport
    # is bound to a closed loop, producing 'RuntimeError: Event loop is
    # closed' on the next call. Forcing each request to open and close its
    # own connection inside the current loop avoids that.
