...
class InferenceAPIManager(object):
    ...
    def get(self, model_name: str) -> InferenceAPIInfo:
        ...
        res = self._manager._get(f'v1/inferenceapis/{self.project_id}/{model_name}').json()
    ...
    def start(self, model_name: str) -> Dict[str, Any]:
        ...
        if self._manager is None:
            raise ManagementError(msg='Manager not initialized')
        res = self._manager._post(f'v1/inferenceapis/{self.project_id}/{model_name}/start')
        return res.json()
    ...
    def stop(self, model_name: str) -> Dict[str, Any]:
        ...
        if self._manager is None:
            raise ManagementError(msg='Manager not initialized')
        res = self._manager._post(f'v1/inferenceapis/{self.project_id}/{model_name}/stop')
        return res.json()
